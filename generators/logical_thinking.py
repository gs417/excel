"""ロジカルシンキング: 単一HTML（vanilla JS、Noto Sans JP CDN）"""


def generate(output_dir: str):
    html = _build_html()
    path = f"{output_dir}/logical-thinking/ロジカルシンキング演習.html"
...
.lesson-header:hover { opacity: 0.9; }
.lesson-header .badge {
...
.textbook p, .textbook li {
  font-size: 0.9rem;
  color: var(--text);
  line-height: 1.85;
  margin: 8px 0;
}
.textbook ul { padding-left: 20px; margin: 8px 0; }
.textbook li { margin: 4px 0; }
.tb-highlight {
  background: #FFF8E1;
...
@media (max-width: 600px) {
  ...
  .textbook { padding: 20px 16px 16px; }
}
.tb-bad, .tb-good {
  border-radius: 8px;
  padding: 14px 16px;
...
.question p { margin-bottom: 12px; font-weight: 500; }
.options label {
  display: block;
  padding: 8px 12px;
  margin: 4px 0;
...
.tree-btn:hover { background: #E8F0FE; }
.tree-btn.remove { color: var(--red); }
.model-answer {
  background: var(--bg);
...
.model-answer h4 { color: var(--accent); margin-bottom: 8px; }
.model-answer.show { display: block; }
.model-tree { margin-left: 0; font-size: 0.9rem; }
.model-tree li { margin: 4px 0; list-style: none; }
.model-tree li::before { content: "├─ "; color: var(--accent); }
...
.reset-btn:hover { background: #FFC7CE; color: var(--red); border-color: var(--red); }
</style>
</head>
<body>

<h1>ロジカルシンキング演習</h1>
<p class="subtitle">全16問 — 進捗はブラウザに自動保存されます</p>

<div class="progress-bar">
  <div class="progress-fill" id="progressFill" style="width:0%"></div>
  <div class="progress-text" id="progressText">0 / 16 完了</div>
</div>

<!-- ===== Lesson 1: MECE ===== -->
<div class="lesson" id="lesson1">
  <div class="lesson-header" onclick="toggleLesson(1)">
    Lesson 1: MECE（漏れなくダブりなく）
    <span class="badge" id="badge1">0/5</span>
  </div>
  <div class="lesson-body" id="body1">
...
<div class="lesson" id="lesson2">
  <div class="lesson-header" onclick="toggleLesson(2)">
    Lesson 2: ロジックツリー
    <span class="badge" id="badge2">0/4</span>
  </div>
  <div class="lesson-body hidden" id="body2">
    ...
    <p style="margin-bottom:16px;color:var(--text-light)">
      それでは練習です。テーマに対してロジックツリーを作成してください。
      ノードを追加・削除して構造化しましょう。
    </p>
...
<div class="lesson" id="lesson3">
  <div class="lesson-header" onclick="toggleLesson(3)">
    Lesson 3: So What? / Why So?
    <span class="badge" id="badge3">0/4</span>
  </div>
  <div class="lesson-body hidden" id="body3">
    ...
    <p style="margin-bottom:16px;color:var(--text-light)">
      それでは練習です。事実→結論、結論→根拠の力を試してみましょう。
    </p>
...
<div class="lesson" id="lesson4">
  <div class="lesson-header" onclick="toggleLesson(4)">
    Lesson 4: 総合演習
    <span class="badge" id="badge4">0/3</span>
  </div>
  <div class="lesson-body hidden" id="body4">
    ...
    <p style="margin-bottom:16px;color:var(--text-light)">
      架空のシナリオに対して、MECE分解 → So What → 改善提案のステップで取り組んでください。
    </p>
    <div id="synthesis-section"></div>
  </div>
</div>

<button class="reset-btn" onclick="resetAll()">全進捗をリセット</button>
...
// ===== State =====
let state = JSON.parse(localStorage.getItem('lt-state') || '{}');
if (!state.mece) state.mece = {};
if (!state.tree) state.tree = {};
if (!state.sowhat) state.sowhat = {};
if (!state.synthesis) state.synthesis = { step: 0, answers: {} };

function saveState() {
  localStorage.setItem('lt-state', JSON.stringify(state));
  updateProgress();
}
...
function countCompleted() {
  let count = 0;
  // MECE: 5
  for (let i = 0; i < 5; i++) if (state.mece[i] !== undefined) count++;
  // Tree: 4 (2 build + 2 compare = counted as 4)
  for (let i = 0; i < 2; i++) if (state.tree[i]) count++;
  count += Object.keys(state.tree).filter(k => state.tree[k] === 'compared').length;
  // SoWhat: 4
  for (let i = 0; i < 4; i++) if (state.sowhat[i] !== undefined) count++;
  // Synthesis: 3 steps
  count += Object.keys(state.synthesis.answers).length;
...
function updateProgress() {
  ...
  document.getElementById('progressText').textContent = done + ' / 16 完了';

  // Badges
  let m = 0; for (let i=0;i<5;i++) if(state.mece[i]!==undefined) m++;
  document.getElementById('badge1').textContent = m + '/5';

  let t = 0; for (let i=0;i<2;i++) { if(state.tree[i]) t++; if(state.tree[i]==='compared') t++; }
  document.getElementById('badge2').textContent = Math.min(t,4) + '/4';

  let s = 0; for (let i=0;i<4;i++) if(state.sowhat[i]!==undefined) s++;
  document.getElementById('badge3').textContent = s + '/4';

  document.getElementById('badge4').textContent = Object.keys(state.synthesis.answers).length + '/3';
}
...
// ===== Lesson 1: MECE =====
function renderMece() {
  const container = document.getElementById('mece-questions');
  container.innerHTML = '';
  meceQuestions.forEach((q, qi) => {
    ...
    const opts = div.querySelector('#mece-opts-'+qi);
    q.options.forEach((opt, oi) => {
      const label = document.createElement('label');
      label.innerHTML = '<input type="radio" name="mece'+qi+'" value="'+oi+'"' +
        (state.mece[qi] !== undefined && state.mece[qi] == oi ? ' checked' : '') + '> ' + opt;
      opts.appendChild(label);
    });

    if (state.mece[qi] !== undefined) showMeceFeedback(qi, state.mece[qi]);
  });
}

function checkMece(qi) {
  const sel = document.querySelector('input[name="mece'+qi+'"]:checked');
  if (!sel) return;
...
function showMeceFeedback(qi, val) {
  const q = meceQuestions[qi];
  const fb = document.getElementById('mece-fb-'+qi);
  const isCorrect = val === q.answer;
  fb.className = 'feedback show ' + (isCorrect ? 'correct' : 'wrong');
  fb.textContent = (isCorrect ? '✓ 正解！' : '✗ 不正解。') + ' ' + q.explanation;

  const opts = document.querySelectorAll('#mece-opts-'+qi+' label');
...
// ===== Lesson 2: Logic Tree =====
function renderTree() {
  const container = document.getElementById('tree-questions');
  container.innerHTML = '';
  treeQuestions.forEach((tq, ti) => {
    const div = document.createElement('div');
    div.className = 'question';
...
function getTreeData(ti) {
  if (!state.tree['data_'+ti]) {
    state.tree['data_'+ti] = { label: '', children: [{ label: '', children: [] }] };
  }
  return state.tree['data_'+ti];
}

function renderTreeNode(ti) {
  const container = document.getElementById('tree-container-'+ti);
  container.innerHTML = '';
  const tree = getTreeData(ti);
...
function buildTreeDOM(node, ti, path) {
  ...
  input.oninput = function() {
    ...
    state.tree['data_'+ti] = getTreeData(ti);
    saveState();
  };
  ...
  if (path.length > 0) {
    ...
    rmBtn.onclick = function() {
      const parent = getNodeByPath(getTreeData(ti), path.slice(0, -1));
  ...
  if (node.children) {
    node.children.forEach((child, ci) => {
      div.appendChild(buildTreeDOM(child, ti, [...path, ci]));
    });
  }
  return div;
}
...
function renderModelAnswer(ti) {
  const ma = treeQuestions[ti].modelAnswer;
  const container = document.getElementById('tree-model-'+ti);
...
function renderSoWhat() {
  const container = document.getElementById('sowhat-questions');
  container.innerHTML = '';
  sowhatQuestions.forEach((q, qi) => {
    const div = document.createElement('div');
    div.className = 'question';
    ...
    const opts = div.querySelector('#sw-opts-'+qi);
    q.options.forEach((opt, oi) => {
      const label = document.createElement('label');
      label.innerHTML = '<input type="radio" name="sw'+qi+'" value="'+oi+'"' +
        (state.sowhat[qi] !== undefined && state.sowhat[qi] == oi ? ' checked' : '') + '> ' + opt;
      opts.appendChild(label);
    });

    if (state.sowhat[qi] !== undefined) showSoWhatFeedback(qi, state.sowhat[qi]);
  });
}

function checkSoWhat(qi) {
  const sel = document.querySelector('input[name="sw'+qi+'"]:checked');
  if (!sel) return;
  const val = parseInt(sel.value);
  state.sowhat[qi] = val;
  saveState();
...
function showSoWhatFeedback(qi, val) {
  const q = sowhatQuestions[qi];
  const fb = document.getElementById('sw-fb-'+qi);
  const isCorrect = val === q.answer;
  fb.className = 'feedback show ' + (isCorrect ? 'correct' : 'wrong');
  fb.textContent = (isCorrect ? '✓ 正解！' : '✗ 不正解。') + ' ' + q.explanation;

  const opts = document.querySelectorAll('#sw-opts-'+qi+' label');
  opts.forEach((label, i) => {
    label.className = '';
    if (i === q.answer) label.className = 'correct';
    else if (i === val && !isCorrect) label.className = 'wrong';
...
function renderSynthesis() {
  const container = document.getElementById('synthesis-section');
  container.innerHTML = '';
  ...
  for (let i = 0; i < 3; i++) {
    ...
    dot.className = 'step-dot' + (i === state.synthesis.step ? ' active' : '') +
      (state.synthesis.answers[i] ? ' done' : '');
    dot.textContent = i + 1;
    indicators.appendChild(dot);
  }
  container.appendChild(indicators);

  const step = synthesisSteps[state.synthesis.step];
  const div = document.createElement('div');
  div.className = 'question';
  const scenarioText = step.scenario.replace(/\\\\n/g, '<br>');
  div.innerHTML = '<h3 style="color:var(--accent);margin-bottom:8px">' + step.title + '</h3>' +
    ...
    (state.synthesis.answers[state.synthesis.step] || '') + '</textarea>' +
  ...
  if (state.synthesis.step > 0) {
    const prev = document.createElement('button');
    prev.className = 'check-btn';
    prev.style.background = 'var(--border)';
    prev.style.color = 'var(--text)';
    prev.textContent = '← 前のステップ';
    prev.onclick = () => { state.synthesis.step--; saveState(); renderSynthesis(); };
    nav.appendChild(prev);
  } else {
    nav.appendChild(document.createElement('span'));
  }
  if (state.synthesis.step < 2) {
    const next = document.createElement('button');
    next.className = 'check-btn';
    next.textContent = '次のステップ →';
    next.onclick = () => { state.synthesis.step++; saveState(); renderSynthesis(); };
    nav.appendChild(next);
  }
  container.appendChild(nav);

  if (state.synthesis.answers[state.synthesis.step]) {
    document.getElementById('synth-model').classList.add('show');
  }
}

function saveSynthStep() {
  const val = document.getElementById('synth-answer').value.trim();
  if (!val) return;
  state.synthesis.answers[state.synthesis.step] = val;
  saveState();
  document.getElementById('synth-model').classList.add('show');
}
...
function resetAll() {
  ...
  localStorage.removeItem('lt-state');
  state = { mece: {}, tree: {}, sowhat: {}, synthesis: { step: 0, answers: {} } };
  init();
}
